    ...
    def new_aws_state(self, target_env_type, new_version=None, new_arn=None):
        ...
        for (tier_type, tier) in self.old_aws_state.envs[target_env_type].items():
            ...
            if new_version is not None:
                new_tier = dataclasses.replace(new_tier, parsed_version=parsed_version)
            if new_arn is not None:
                # A new ARN implies a new environment
                new_env_num = max([(tier.num + 1) % 1000 for tier in self.old_aws_state.envs[target_env_type].values()])
                worker_name = (AwsTierType.WORKER.value + "-") if tier_type is AwsTierType.WORKER else ''
                new_tier = dataclasses.replace(
                    new_tier,
                    platform_arn=new_arn,
                    environment_id=None,
                    name=f'safecast{self.old_aws_state.aws_app_name}-{target_env_type.value}-{worker_name}{new_env_num:03}',
                    num=new_env_num
                )
            new_envs[target_env_type][tier_type] = new_tier
        return dataclasses.replace(self.old_aws_state, envs=new_envs)
